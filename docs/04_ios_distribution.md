...
| データ送信 | 記録した軌跡・スポット・写真は全て端末内。逆ジオコーディングはApple純正CLGeocoderのみ使用。地図描画にGoogle Maps SDKを使う場合、表示中の地図範囲がGoogleに渡る(地図描画に不可避な通信。App Store のプライバシー表示では「位置情報 / アプリの機能」として申告する) |
